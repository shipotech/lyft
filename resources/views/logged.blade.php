@extends('layout')

@section('title', 'Lyft')
@section('bodyClass', 'class=full-height')

@section('header')
    <header class="white-text purple-gradient h-100">
        <!-- Jumbotron -->

        <div class="container h-100">
            <!-- Grid row -->
            <div class="row h-100">

                <div class="col-md-6 h-100">
                    <div class="container-fluid h-100 white px-4">
                        <a href="{{ route('user.session') }}" class="inline-block text-md-left">
                            <svg fill="currentColor" preserveAspectRatio="xMidYMid meet" height="4em" width="4em"
                                 viewBox="0 0 72 50" class="img-fluid ml-2 mt-3 text-secondary"
                                 style="vertical-align: middle;">
                                <g>
                                    <g>
                                        <path d="M.023.888h10.912v28.716c0 4.544 2.12 7.251 3.796 8.411-1.775 1.547-7.198 2.901-11.24-.386C1.106 35.69.022 32.504.022 29.507V.888zM68.218 24.742v-3.018h3.328V11.137H67.89C66.488 4.781 60.717.018 53.815.018c-7.95 0-14.396 6.32-14.396 14.116v24.724c2.265.312 4.964-.04 7.26-1.906 2.384-1.938 3.467-5.124 3.467-8.122v-.91h5.45V17.333h-5.45v-3.199h.013c0-1.98 1.637-3.585 3.656-3.585 2.02 0 3.662 1.605 3.662 3.585v10.608c0 7.796 6.453 14.116 14.403 14.116V28.326c-2.019 0-3.662-1.605-3.662-3.584"></path>
                                        <path d="M26.89 11.137v15.537c0 .883-.746 1.597-1.667 1.597-.92 0-1.667-.714-1.667-1.597V11.137H12.759V29.41c0 3.288 1.137 7.445 6.31 8.799 5.18 1.355 8.185-1.45 8.185-1.45-.274 1.848-2.049 3.201-4.909 3.491-2.164.22-4.93-.483-6.31-1.063v9.681c3.517 1.017 7.236 1.345 10.877.653 6.606-1.257 10.775-6.671 10.775-13.874v-24.51H26.891z"></path>
                                    </g>
                                </g>
                            </svg>
                        </a>

                        <div class="row">
                            <div class="col-12 text-dark px-4">
                                <h1 class="h1-responsive font-weight-bold pt-5">Bienvenido</h1>
                                <p class="lead">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Cumque ducimus
                                    incidunt quisquam! Atque autem cumque, fugiat in itaque natus possimus unde
                                    voluptate! At delectus dolore minima nemo quasi, quis voluptatum!</p>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
            <!-- Grid row -->
        </div>

        <!-- Jumbotron -->
    </header>
@endsection

@if(session('message_logged'))
    @section('scripts')
        <script type="text/javascript">
            $(document).ready(function () {
                toastr.success('{{ session('message_logged') }}');
            });
        </script>
    @endsection
    @php
        // Delete session
        session(['message_logged' => false]);
    @endphp
@endif